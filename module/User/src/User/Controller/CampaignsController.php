...
class CampaignsController extends CoreController
{
    public function indexAction()
    {
        $designs = $this->getRepository('Campaign')->searchByUserStatusesTitle($this->getUser());
    ...
    {
        return new ViewModel([
            'form' => $this->createForm(new Campaign()),
        ]);
    }
    ...
    public function publishAction()
    {
        ...
        if ($request->isPost()) {
            /** @var $design Design */
            if ($this->params()->fromRoute('id')) {
                $design = $this->getRepository('Design')->findByIdA
                ndUser($this->params()->fromRoute('id'), $this->getUser());
            } else {
                $design = $this->getRepository('Design')->findOneNewByUser($this->getUser());
            }
            $form = $this->createForm($design);
